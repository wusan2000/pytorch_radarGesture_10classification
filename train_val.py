...
from data_rar import train_dataloader, train_datasets, val_datasets, val_dataloader
import cfg
...
def test():
    ...
    print('ACC in val: %.3f' % (total_correct / (len(val_dataloader) * batch_size)))
...
for iteration in range(start_iter, max_iter):
    ...
    if iteration % epoch_size == 0:
        ...
        if epoch % 1 == 0 and epoch > 0:
            if cfg.GPUS > 1:
                ...
                print(f"save checkpoint in {save_folder}/epoch_{epoch}.pth")
            else:
    ...
    train_acc = (train_correct.float()) / batch_size        # because batch_size choose
