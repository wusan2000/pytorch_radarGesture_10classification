...
import cfg
from data_rar import tta_test_transform, get_test_transform
...
def predict(model):
    ...
    return _id, pred_list, pred_list_arg
...
if __name__ == "__main__":
    ...
    _id, pred_list, pred_list_arg = predict(trained_model)
    ...
    submission = pd.DataFrame({"ID": _id, "Label": pred_list, "Label_arg": pred_list_arg})
    submission.to_csv(cfg.BASE + '{}_submission_arg.csv'
                      .format(model_name), index=False, header=False)
    print(f"save in dir:{cfg.BASE} + '{model_name}_submission_arg.csv'")
    print("val:")
    with open(cfg.VAL_LABEL_DIR, 'r') as f:
        imgs = f.readlines()

    # _id, pred_list = tta_predict(trained_model)
    _id, pred_list, _ = predict(trained_model)

    # submission = pd.DataFrame({"ID": _id, "Label": pred_list})
    # submission.to_csv(cfg.BASE + '{}_submission_val.csv'
    #                   .format(model_name), index=False, header=False)
    print("train:")
    ...
    # submission = pd.DataFrame({"ID": _id, "Label": pred_list})
    # submission.to_csv(cfg.BASE + '{}_submission_train.csv'
    #                   .format(model_name), index=False, header=False)
